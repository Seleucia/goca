...
def pyav_decode(
    ...
    if duration is None or decode_full_vid==True:
...
def pyav_decode_full(
    ...
    # If video stream was found, fetch video frames from the video.
    if container.streams.video:
        video_frames, max_pts = pyav_decode_stream_full(
            container,
            video_start_pts,
            video_end_pts,
            container.streams.video[0],
    ...
    try:
        ...
        if backend == "pyav":
            frames, spec, fps, decode_all_video = pyav_decode(
                ...
                use_temporal_jittering=use_temporal_jittering,
                z_normalize=z_normalize,
                decode_full_vid=decode_full_vid
            )
...
def decode_full(
    ...
    try:
        # print(backend,aug_audio,decode_audio)
        if backend == "pyav":
            frames, spec, fps, decode_all_video = pyav_decode_full(
                vid_path,
                container,
                sampling_rate,
                num_frames,
