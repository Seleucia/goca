...
import os,socket,pickle, signal, time
import numpy as np
import torch
import random
...
def init_distributed_mode(args):
    ...
    Initialize the following variables:
        ...
        - rank
    """
    # os.environ['MASTER_ADDR'] = 'hcc-g1-compute-0-0'
    # os.environ['MASTER_PORT'] = '4000'
    ...
    # print('gpu_to_work_on',args.world_size, args.gpu_to_work_on ,args.rank , torch.cuda.device_count()	)
    torch.cuda.set_device(args.gpu_to_work_on)
    print('*'*10,args.gpu_to_work_on)

    if args.rank==0 and args.is_slurm_job:
        assert args.master_node==socket.gethostname()
    return
...
def initialize_exp(params, *args, dump_params=True):
    ...
    # dump parameters
    if dump_params:
        pickle.dump(params, open(os.path.join(params.log_path, "params.pkl"), "wb"))

    # create repo to store checkpoints
    params.dump_checkpoints = os.path.join(params.log_path, "checkpoints")
    ...
    if params.rank == 0:
        logger = create_logger(
            os.path.join(params.log_path, "train.log"), rank=params.rank
        )
...
class AverageMeter(object):
    ...
    def synchronize_between_processes(self):
        ...
        self.sync_avg = sum / count
        return self.sync_avg
        # print('*'*100,self.count, count)
